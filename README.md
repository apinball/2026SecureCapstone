...
```
1. 저장소 코드 가져오기
2. Stage 설정 및 nginx.conf 준비
3. 정적 보안 분석 (Trivy CVE · Gitleaks 시크릿 · Semgrep SAST)
4. TLS 암호 정책 검증 — Security Gate (취약 프로토콜/암호 차단)
5. PQC Nginx Docker 빌드 및 가동
6. 배포 후 동적 TLS 검증 (PQC 그룹 협상 확인)
7. CBOM 명세서 생성 및 아티팩트 업로드
```
